...
module.exports.run = (client, message, args) => {
	...
	if(args.length === 0) {
		...
		if(checkWithoutSides(numDiceRolled, maxDice, maxSides) != undefined) return checkWithoutSides(numDiceRolled, maxDice, maxSides);
		return message.channel.send(`**Result:** ${generateDiceResult(numDiceRolled, defaultSides)}`);
	}
	else if(args.length === 2) {
		//amount of dice and sides
		//when the second key in array does not contain any letters, only numbers
		if(args[1].match(/[a-z]/) === null) {
			const numDiceRolled = Number(args[0]);
			const sides = Number(args[1]);
			if(checkWithSides(numDiceRolled, sides, maxDice, maxSides) != undefined) return checkWithSides(numDiceRolled, sides, maxDice, maxSides);
			return message.channel.send(`**Result:** ${generateDiceResult(numDiceRolled, defaultSides)}`);
		}
